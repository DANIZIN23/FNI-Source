...
class PlayState extends MusicBeatState
{
	...
	public var spoopyvignette:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				var bg:BGSprite = new BGSprite('bg/stageback', -820, -915);
				bg.setGraphicSize(Std.int(bg.width * 1.31));
				bg.updateHitbox();
				add(bg);

				var stageFront:BGSprite = new BGSprite('bg/stagefront', -808, 800);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.4));
				stageFront.updateHitbox();
				add(stageFront);

			case 'evilstage': //Week 1 but evil

				var bg:BGSprite = new BGSprite('bg/trollgestage', -820, -535);
				bg.setGraphicSize(Std.int(bg.width * 1.31));
				bg.updateHitbox();
				add(bg);

			case 'aurorawoods': //Week 2 but awesome

				var bg:BGSprite = new BGSprite('bg/aurorawoods', -820, -535);
				bg.setGraphicSize(Std.int(bg.width * 1.31));
				bg.updateHitbox();
				add(bg);

				jumpScare = new FlxSprite().loadGraphic(Paths.image('auroraJumpscare'));
			...
			case 'pibbystage': 
				...
				pibbybg = new BGSprite('bg/pibbystage', -820, -915, ['trippy movin'], true);
				pibbybg.setGraphicSize(Std.int(pibbybg.width * 1.31));
				pibbybg.updateHitbox();
				add(pibbybg);

				bgGlitch = new BGSprite('bg/pibbystage', -820, -915, ['glitch'], true);
				bgGlitch.setGraphicSize(Std.int(bgGlitch.width * 1.31));
		...
		if (SONG.song.toLowerCase() == 'rainbow-trololo')
			gfVersion = 'eduardgf';
		...
		switch(curStage)
		{
			...
			case 'pibbystage':
				if (SONG.player1 == 'bf-scared' && gfVersion == 'gf-scared')
				{
					boyfriend.x -= 250;
					boyfriend.y += 15;
					gf.x += 60;
		...
		#end
		if (SONG.song.toLowerCase() == 'evacuate')
		{
			vignette = new FlxSprite().loadGraphic(Paths.image('vignette'));
			vignette.setGraphicSize(Std.int(vignette.width * 2.8));
			vignette.width = 1280;
			vignette.height = 720;
			vignette.x = -900;
			vignette.y = -200;
		...
		{
			vignette = new FlxSprite().loadGraphic(Paths.image('vignette'));
			vignette.setGraphicSize(Std.int(vignette.width * 2.9));
			vignette.width = 1280;
			vignette.height = 720;
			vignette.x = -580;
			vignette.y = -400;
			...
			spoopyvignette = new FlxSprite().loadGraphic(Paths.image('spoopyvignette'));
			spoopyvignette.setGraphicSize(Std.int(spoopyvignette.width * 1.31));
			//spoopyvignette.width = 1280;
			//spoopyvignette.height = 720;
			spoopyvignette.x = 100;
			spoopyvignette.y = -250;
			spoopyvignette.updateHitbox();
			spoopyvignette.alpha = 1;
			add(spoopyvignette);
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == "evacuate" && storyDifficulty > 1 && inCutscene == false)
				health -= 0.000225;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Pibby BG Glitch':
				if(curStage == 'pibbystage') 
				{
					bgGlitch.dance(true);
					bgGlitch.visible = true;
				}
	...
	function jumpscare(duration:Float) 
	{
		...
		camHUD.shake(0.01015625, (((!Math.isNaN(duration)) ? duration : 1) * Conductor.stepCrochet) / 1000, function()
			{
				jumpScare.visible = false;
	...
	public function moveCamera(isDad:Bool) {
		...
		if(isDad) {
			if (SONG.song.toLowerCase() == 'aurora')
				camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y);
			else
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (storyWeek == 0)
					{
						...
						{
							startVideo('post-evacuate');
							//if (curStep == 210 || curBeat == 210)
							//	LoadingState.loadAndSwitchState(new StoryMenuState());
						}
						/*else if (PlayState.storyDifficulty == 3)
						{
							startVideo('troll');
							if (!inCutscene)
								LoadingState.loadAndSwitchState(new Killer());
						}*/
					}
					if (storyWeek == 1)
						{
							startVideo('omghedead');
							//if (curStep == 210 || curBeat == 210)
							//	LoadingState.loadAndSwitchState(new StoryMenuState());
						}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		else if(daNote.noteType == 'Axe Note') 
		{
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'aurorawoods':
				if (curBeat == 16)
					FlxTween.tween(spoopyvignette, {alpha: 0}, 1);
				switch (curBeat)
				{
					case 48, 80, 112, 142, 176, 209, 240, 272, 304, 336, 368, 400, 432:
						defaultCamZoom = 0.425;
